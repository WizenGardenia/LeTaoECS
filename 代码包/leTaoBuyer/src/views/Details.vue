...
<template>
  <div id="details">
    ...
    <div class="page-header">
      <div class="title">
        <p>{{productDetails[0].cmdtyName}}</p>
        <!-- <div class="list">
          <ul>
            <li>
    ...
    <div class="main">
      <!-- 左侧商品轮播图 -->
      <div class="block">
        <el-carousel height="560px" v-if="1">
          <el-carousel-item v-for="item in productDetails" :key="item.cmdtyName">
            <!-- <img style="height:560px;" :src="$target + item.product_picture" :alt="item.intro" /> -->
            <img style="height:560px;" :src="item.cmdtyPrev" :alt="item.cmdtyDetailsId" />
          </el-carousel-item>
        </el-carousel>
      ...
      <!-- 右侧内容区 -->
      <div class="content">
        <h1 class="name">{{productDetails[0].cmdtyName}}</h1>
        <p class="intro">{{productDetails[0].cmdtyPrmt}}</p>
        <!-- <p class="store">小米自营</p> -->
        <div class="price">
          <span>{{productDetails[0].cmdtyDetails}}元</span>
          <!-- <span
        ...
        <div class="pro-list">
          <span class="pro-name">{{productDetails[0].cmdtyName}}</span>
          ...
          <p class="price-sum">总计 : {{productDetails[0].cmdtyDetails}}元</p>
        </div>
        <!-- 内容区底部按钮 -->
        <div class="button">
...
export default {
  ...
  methods: {
    ...
    getDetails(val) {
      ...
      dateFormat.append('cmdty_id',val);
      this.$axios
        .post("/api/cmdty-details/showCmdDetails", 
        dateFormat
        // {
        //   productID: val
        // }
    ...
    addShoppingCart() {
      ...
      const dateFormat = new FormData();
      dateFormat.append('user_ID',this.$store.getters.getUser.user_id)
      dateFormat.append('cmdty_ID',this.productID)
      this.$axios
        .post("/api/cart/addCommodity", dateFormat)
        .then(res => {
