    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.cartId">
          ...
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {productID:item.cmdtyId} }">
              <!-- <img :src="$target + item.productImg" /> -->
              <img :src="item.cmdtyPrev" />
            </router-link>
          </div>
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {productID:item.cmdtyId} }"
            >{{item.cmdtyName}}</router-link>
          </div>
          <!-- <div class="pro-price">{{item.price}}元</div> -->
          <div class="pro-price">{{item.cmdtyDetails}}元</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.cmdtyNum"
              @change="handleChange($event,index,item.cmdtyId)"
              :min="1"
              :max="item.maxNum"
            ></el-input-number>
          </div>
          <div class="pro-total pro-total-in">{{item.cmdtyDetails*item.cmdtyNum}}元</div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.cartId,item.cmdtyId)"
                >确定</el-button>
              </div>
...
export default {
  ...
  methods: {
    ...
    handleChange(currentValue, key, productID) {
      ...
      dateFormat.append('cmdty_ID',productID)
      dateFormat.append('cmdty_num',currentValue)
      // alert(this.$store.getters.getUser.user_id)
      // alert(productID)
      // alert(currentValue)
      this.$axios
        .post("/api/cart/updateCart", dateFormat)
        .then(res => {
    ...
    //  alert(productID)
      this.$axios
        .post("/api/cart/delCart", dateFormat)
        .then(res => {
          switch (res.data.code) {
            case "1":
              // “001” 删除成功
