        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.id">
              ...
              <span class="pro-name">{{item.cmdtyName}}</span>
              <span class="pro-price">{{item.cmdtyDetails}}元 x {{item.cmdtyNum}}</span>
              <span class="pro-status"></span>
              <span class="pro-total">{{item.cmdtyDetails * item.cmdtyNum}}元</span>
            </li>
          </ul>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    addOrder() {
    //  alert(this.getCheckGoods[0].cmdtyName)
    let Obj = JSON.stringify(this.getCheckGoods);
    const dateFormat=new FormData();
    dateFormat.append('cartCommodities',Obj)
    dateFormat.append('user_ID',this.$store.getters.getUser.user_id)
      this.$axios 
        .post("/api/order-form/addOrders", dateFormat)
        .then(res => {
          ...
          switch (res.data.code) {
            ...
            case "1":
              for (let i = 0; i < products.length; i++) {
                ...
                // 删除已经结算的购物车商品
                this.deleteShoppingCart(temp.cmdtyId);
              }
              // 提示结算结果
