  ...
  methods: {
    ...
    Login() {
      const dateFormat = new FormData();
       dateFormat.append('username',this.LoginUser.name)
       dateFormat.append('password',this.LoginUser.pass)
      // 通过element自定义表单校验规则，校验用户输入的用户信息
      this.$refs["ruleForm"].validate(valid => {
        //如果通过校验开始登录
        if (valid) {
        //  alert("----------->"+this.LoginUser.name);
        //  alert("----------->"+this.LoginUser.pass);
       
           this.$axios
             .post("/api/user/login", dateFormat
          )
            .then(res => {
